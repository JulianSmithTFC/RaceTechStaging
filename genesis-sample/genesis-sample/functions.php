...
function checkout_fix_scripts() {
    wp_enqueue_script( 'bootstrap_jquery', 'https://cdnjs.cloudflare.com/ajax/libs/jquery/3.4.1/jquery.min.js');
    wp_enqueue_script( 'checkout-fixes', get_stylesheet_directory_uri() . '/js/checkout-fix.js');
}
add_action( 'wp_enqueue_scripts', 'checkout_fix_scripts' );
...
// Register and Enqueue reCaptcha Script

	function enqueue_recaptcha_script() {
		wp_register_script( 'Google reCaptcha', 'https://www.google.com/recaptcha/api.js', null, null, false );
		wp_enqueue_script('Google reCaptcha');
	}

	add_action( 'wp_enqueue_scripts', 'enqueue_recaptcha_script' );

// Add reCAPTCHA field into the registration form

		function woocommerce_edit_registration_form() {
            ?>
            <p id="recaptcha" class="g-recaptcha" data-sitekey="6LcERCwbAAAAACdS1AyGCGSO-rSso4w0ZbNJviin"></p>
            <?php
        }
        add_action( 'woocommerce_register_form', 'woocommerce_edit_registration_form', 10 );

    /**
     * Validate Woocommerce Registration form fields
     */

    function validate_extra_register_fields( $errors, $username, $email ) {
        if ( empty( $_POST['g-recaptcha-response'] ) ) {
                $errors->add( 'captcha-error', wp_kses_post( ' Captcha is missing.'));
        }
        return $errors;
    }
    add_filter( 'woocommerce_registration_errors', 'validate_extra_register_fields', 10, 3 );

	// Remove reCaptcha from checkout page

	function remove_recaptcha_on_checkout() {
			if (is_checkout()) {
				remove_action('woocommerce_register_form', 'woocommerce_edit_registration_form');
				remove_filter('woocommerce_registration_errors', 'validate_extra_register_fields');
				wp_dequeue_script( 'Google reCaptcha' );
			}
		}

	add_action( 'woocommerce_review_order_before_submit', 'remove_recaptcha_on_checkout', 1);
