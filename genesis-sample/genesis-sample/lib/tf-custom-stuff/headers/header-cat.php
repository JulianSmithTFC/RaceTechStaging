...
//require (get_stylesheet_directory() . '/lib/tf-custom-stuff/cat-menu.php');
                    ...
                    <div class="container-fluid header-before-container">
                        <div class="row">
                            <div class="col-lg-5 col-md-12 col-sm-12">
                                <span id="desktop-header-search">
                                    <?php echo do_shortcode( '[woocommerce_product_search show_description="no" category_results="no"]' ); ?>
                                </span>
                            </div>
                                ...
                                <div class="container-fluid" align="right">
                                    <div id="cartPopupContainer" class="d-none " align="left">
										...
										<div class="cart-contents">
											<p>
												Total Products: <?php echo sprintf ( _n( '%d item', '%d items', WC()->cart->get_cart_contents_count() ), WC()->cart->get_cart_contents_count() ); ?>
											</p>
											<p>
												Total Cost: <?php echo WC()->cart->get_cart_total(); ?>
											</p>
										</div>
                        ...
                        <?php if (!empty($first_item['secondLevel'])){ ?>
                            <?php foreach ($first_item['secondLevel'] as $item) { ?>
                                <?php if(!empty($item['thirdLevel'])){ ?>
                                    <li id="dropdown-link-<?php echo $item['ID'] ?>" class="nav-item my-header-navbar-li dropdown-link-<?php echo $item['ID'] ?> dropdown-link">
                                        <?php if($item['object'] == 'product_cat'){ ?>
                                            <span class="nav-link text-uppercase my-header-navbar-links" ><?php echo get_term( (int)$item['object_id'] )->name; ?></span>
                                        <?php }elseif ($item['object'] == 'custom'){ ?>
                                            <span class="nav-link text-uppercase my-header-navbar-links" ><?php echo $item['name']; ?></span>
                                        <?php } ?>
                                    </li>
                                <?php }else{ ?>
        ...
        <div class="my-header-megamenu-containerInner">
            <?php foreach ($parentArray as $first_item){ ?>
                <?php if (!empty($first_item['secondLevel'])){ ?>
                    <?php foreach ($first_item['secondLevel'] as $item) { ?>
                        <?php if(!empty($item['thirdLevel'])){ ?>
                            <div id="dropdown-container-<?php echo $item['ID'] ?>" class="d-none dropdown-container drop-downs">
                                <div class="container ">
                                    <div align="center">
                                        <div align="right">
                                            <span class="dropdown-exit" ><i class="fas fa-times"></i></span>
                                        </div>
                                    ...
                                    <div class="row">
                                        <?php foreach ($item['thirdLevel'] as $subParent) { ?>
...
<script>
    ...
    $( "#dropdown-link-<?php echo $item['ID'] ?>" ).click(
        function() {
            $( ".dropdown-container" ).not("#dropdown-container-<?php echo $item['ID'] ?>").addClass( "d-none" );
            $( "#dropdown-container-<?php echo $item['ID'] ?>" ).toggleClass( "d-none" );
        }
    );
    ...
    $(document).mouseup(function (e) {
        if (($(e.target).closest(".dropdown-container").length === 0) && ($(e.target).closest(".dropdown-link").length === 0)) {
            $(".dropdown-container").addClass( "d-none" );
        }
    });
   ...
   //     var sourceSearch = '';
   //
   //     if ($("#mobile-header-search").html().length > 0) {
   //         sourceSearch = $("#mobile-header-search");
   //     }
   //
   //     if ($("#desktop-header-search").html().length > 0) {
   //         sourceSearch = $("#desktop-header-search");
   //     }
   //
   //     console.log('changed');
   //
   //     //Extra Small
   //     if($(window).width() < 576) {
   //         sourceSearch.contents().appendTo('#mobile-header-search');
   //     }
   //     //Small
   ...
   //         sourceSearch.contents().appendTo('#desktop-header-search');
   //     }
   //     //Extra Large
